(function (factory) {
    if (typeof window.define === 'function') {
        if (window.define.amd) {
            // AMD模式
            window.define('JEditor', ["jquery"], factory);
    ...
    // 验证是否引用jquery
    if (!$ || !$.fn || !$.fn.jquery) {
        alert('在引用JEditor.js之前，先引用jQuery，否则无法使用 JEditor');
        return;
    }
...
(function (window, $) {
    if (window.JEditor) {
        // 重复引用
        alert('一个页面不能重复引用 JEditor.js 或 JEditor.min.js ！！！');
        return;
    }
    ...
    // 用户在引用 JEditor.js 之后，还可以通过 E.plugin() 注入自定义函数，
...
_e(function (E, $) {
    ...
    MenuContainer.fn.init = function () {
        var self = this;
        var $menuContainer = $('<div class="JEditor-menu-container clearfix"></div>');
    ...
    MenuContainer.fn.changeShadow = function () {
        ...
        $txt.on('scroll', function () {
            if ($txt.scrollTop() > 10) {
                $menuContainer.addClass('JEditor-menu-shadow');
            } else {
                $menuContainer.removeClass('JEditor-menu-shadow');
            }
        });
    };
...
_e(function (E, $) {
    ...
    DropList.fn.initDOM = function () {
        ...
        var $list = $('<div class="JEditor-drop-list clearfix"></div>');
...
_e(function (E, $) {
    ...
    DropPanel.fn.initDOM = function () {
        ...
        var $panel = $('<div class="JEditor-drop-panel clearfix"></div>');
...
_e(function (E, $) {
    ...
    Modal.fn.initDom = function () {
        var self = this;
        var $content = self.$content;
        var $modal = $('<div class="JEditor-modal"></div>');
        var $close = $('<div class="JEditor-modal-close"><i class="jeditor-icon-cancel-circle"></i></div>');
...
_e(function (E, $) {
    ...
    Txt.fn.init = function () {
        ...
        if ($valueContainer.get(0).nodeName === 'DIV') {
            ...
            $txt = $valueContainer;
            $txt.addClass("JEditor-txt");
            $txt.attr('contentEditable', 'true');
        } else {
            // 如果不是div（是textarea），则创建一个div
            $txt = $(
                '<div class="JEditor-txt" contentEditable="true">' +
                    currentValue +
                '</div>'
            );
        }
    ...
    Txt.fn.bindPasteFilter = function () {
        ...
        $txt.on('paste', function (e) {
            ...
            var blob = items[itemLength].getAsFile();
            ...
            if (data && data.getData) {
                ...
                pasteHtml = '<p style="text-align: left" >' + pasteHtml + '</p>';
                pasteHtml = pasteHtml.replace((new RegExp('\n', 'g'), '</p style="text-align: left;"><p>'));
                console.log(pasteHtml)
                // console.log(pasteHtml);
                // if (pasteHtml) {
                //     if(/<body([\s\S]*)<\/body>/g.test(pasteHtml)) {
                //         pasteHtml = pasteHtml.match(/<body([\s\S]*)<\/body>/g)[0];
                //     }
                ...
                // 拼接为 <p> 标签
                resultHtml = '<p style="text-align: left" >' + resultHtml + '</p>';
            ...
            // 执行命令
            if (resultHtml && blob == null) {
                var imageList = [];
                var imageKey;
                var $wrap = $('<div/>').append(resultHtml);
                $.each($wrap.find('img'), function(index, img) {
                    var $img = $(img),
                        imgUrl = $img.attr('src');
                    imageKey = Math.round(Math.random()*(99999-10000 + 1)+10000);
                    imageList.push({
                        key: imageKey,
                        url: imgUrl
                    });
                    $img.attr({'data-upload-key':imageKey, src: 'http://test.m1world.com/page/common/images/loading.gif'});
                });
                resultHtml = $wrap.html();
                ...
                if(imageList.length > 0){
                    imageList.forEach(function(img,index){
                        imageUpload(img.url,img.key,pasteEvent);
                    });
                }
            }
        });
        // 上传
        function imageUpload(url,key,e){
            var uploadFileName = editor.config.uploadImgFileName || 'JEditorPasteFile';
            ...
            //         uploadKey: key,
            //         name: uploadFileName,
            //         success: setImageUploadCallback
            //     });
            // };
        ...
        // 上传完成后返回图片url处理
        function setImageUploadCallback(data,key){
            var $img = $txt.find('img[data-upload-key='+ key +']');
            if($img.length > 0){
                if(data){
                    var json = JSON.parse(data);
                    if(json.data.fileUrl){
                        $img.removeAttr('data-upload-key');
                        $img.attr({'src':json.data.fileUrl});
                    }
...
_e(function (E, $) {
    ...
    // 超时时间
    E.config.uploadTimeout = 5 * 60 * 1000;
    // 上传单个图片最大值    默认2M
    E.config.uploadImageMaxSize = 2 * 1024 * 1024;
    // 上传单个视频最大值    默认200M
    E.config.uploadVideoMaxSize = 200 * 1024 * 1024;
    // 用于存储上传回调事件
    E.config.uploadImgFns = {};
    E.config.uploadVideoFns = {};
    // 自定义上传图片的filename
...
_e(function (E, $) {
     ...
     E.UI.menus = {
        ...
        'default': {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-command"></i></a>',
            selected: '.selected'
        },
        bold: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-bold"></i></a>',
            selected: '.selected'
        },
        underline: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-underline"></i></a>',
            selected: '.selected'
        },
        italic: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-italic"></i></a>',
            selected: '.selected'
        },
        forecolor: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-pencil"></i></a>',
            selected: '.selected'
        },
        bgcolor: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-brush"></i></a>',
            selected: '.selected'
        },
        strikethrough: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-strikethrough"></i></a>',
            selected: '.selected'
        },
        eraser: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-eraser"></i></a>',
            selected: '.selected'
        },
        quote: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-quotes-left"></i></a>',
            selected: '.selected'
        },
        source: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-code"></i></a>',
            selected: '.selected'
        },
        fontfamily: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-font2"></i></a>',
            selected: '.selected'
        },
        fontsize: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-text-height"></i></a>',
            selected: '.selected'
        },
        head: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-header"></i></a>',
            selected: '.selected'
        },
        orderlist: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-list-numbered"></i></a>',
            selected: '.selected'
        },
        unorderlist: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-list-bullet"></i></a>',
            selected: '.selected'
        },
        alignleft: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-align-left"></i></a>',
            selected: '.selected'
        },
        aligncenter: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-align-center"></i></a>',
            selected: '.selected'
        },
        alignright: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-align-right"></i></a>',
            selected: '.selected'
        },
        link: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-link"></i></a>',
            selected: '.selected'
        },
        unlink: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-unlink"></i></a>',
            selected: '.selected'
        },
        table: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-table"></i></a>',
            selected: '.selected'
        },
        emotion: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-happy"></i></a>',
            selected: '.selected'
        },
        img: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-picture"></i></a>',
            selected: '.selected'
        },
        video: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-play"></i></a>',
            selected: '.selected'
        },
        location: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-location"></i></a>',
            selected: '.selected'
        },
        insertcode: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-terminal"></i></a>',
            selected: '.selected'
        },
        undo: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-ccw"></i></a>',
            selected: '.selected'
        },
        redo: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-cw"></i></a>',
            selected: '.selected'
        },
        fullscreen: {
            normal: '<a href="#" tabindex="-1"><i class="jeditor-icon-enlarge2"></i></a>',
            selected: '<a href="#" tabindex="-1" class="selected"><i class="jeditor-icon-shrink2"></i></a>'
        }
     };
...
_e(function (E, $) {
    ...
    E.fn.addEditorContainer = function () {
        this.$editorContainer = $('<div class="JEditor-container"></div>');
    };
        ...
        $.each(configColors, function (k, v) {
            $content.append(
                [
                    ...
                    '    style="color: ' + k + '" ',
                    '><i class="jeditor-icon-pencil"></i></a>'
                ].join('')
            );
        });
        ...
        $.each(configColors, function (k, v) {
            $content.append(
                [
                    ...
                    '    title="' + v + '" commandValue="' + k + '" ',
                    '    style="color: ' + k + '" ',
                    '><i class="jeditor-icon-brush"></i></a>'
                ].join('')
            );
        });
        $content.on('click', 'a[commandValue]', function (e) {
...
_e(function (E, $) {
    ...
    function linkContentHandler (editor, menu, $linkContent) {
        ...
        var $txtPreview = $('<span><i class="jeditor-icon-file-image"></i> 图片预览</span>');
        var $previewContainer = $('<div class="content-preview"></div>');
        $previewContainer.append($txtPreview);
        var $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
        var $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');

        $linkContent.append($urlContainer).append($previewContainer).append($btnSubmit).append($btnCancel);


        $urlInput.bind('input propertychange', function(evnet){
            var url = $.trim($(this).val());

            var img = '<img src="'+ url +'" style="max-width:100%; "/>';
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        var $howToCopy = $('<a href="http://www.m1world.com" target="_blank" style="display:inline-block;margin-top:10px;margin-left:10px;color:#999;">如何复制视频链接？</a>');
        var $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
        var $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        menu.clickEvent = function (e) {
            // 增加样式
            var $editorContainer = editor.$editorContainer;
            $editorContainer.addClass('JEditor-fullscreen');
        ...
        menu.clickEventSelected = function (e) {
            // 取消样式
            var $editorContainer = editor.$editorContainer;
            $editorContainer.removeClass('JEditor-fullscreen');
    ...
    E.plugin(function () {
        ...
        editor.xhrUploadImg = function (opt) {
            ...
            xhr.onload = function () {
                ...
                callbackSuccess(xhr.responseText,uploadKey);
            };
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var $progress = $('<div class="JEditor-upload-progress"></div>');
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        // 自定义UI，并添加到上传dom节点上
        var $uploadIcon = $('<div class="upload-icon-container"><span class="upload-icon-button"><i class="jeditor-icon-upload"></i> 上传本地图片</span></div>');
...
_e(function (E, $) {
    ...
    var UploadFile = function (opt) {
        ...
        this.multiple = opt.multipe === undefined ? true : opt.multipe;
        this.xhrUpload = opt.xhrUpload || this.editor.xhrUploadImg;
        this.uploadFns = opt.uploadFns || this.editor.config.uploadImgFns;
    };
    ...
    UploadFile.fn.selected = function (e, input) {
        var self = this;
        var isHugeFile = false; // 用于判断文件是否超出上传最大值
        ...
        $.each(files, function(index, file){
            ...
            // console.log('文件类型为：' + self.fileAccept);
            // console.log('max image: ' + E.config.uploadImageMaxSize );
            // console.log('max video: ' + E.config.uploadVideoMaxSize );
            // console.log(self.fileAccept.indexOf('video'));
            // console.log(self.fileAccept.indexOf('image'));
            var maxSize = E.config.uploadImageMaxSize / 1024 / 1024;
            var maxSizeMB = maxSize / 1024 / 1024;
            var fileType = file.type;
            var fileTypeName = '图片';
            var fileSize = (file.size / 1024 / 1024).toFixed(2);
            if(fileType.indexOf('video') >= 0){
                maxSize = E.config.uploadVideoMaxSize / 1024 / 1024;
                fileType = '视频';
            }
            ...
            if(fileSize > maxSize){
                isHugeFile = true;
                self.clear();
                alert('您上传的' + fileTypeName + file.name + '大小为：' + fileSize + ' MB 最大可上传 ' + maxSize + ' MB');
                return false;
            }
        });
    ...
    UploadFile.fn.upload = function (file) {
        ...
        var fileType = file.type || '';
        var uploadFns = self.uploadFns || editor.config.uploadImgFns;
        var uploadFileName = editor.config.uploadImgFileName || 'JEditorH5File';
        var onload = uploadFns.onload;
        var ontimeout = uploadFns.ontimeout;
        var onerror = uploadFns.onerror;
        ...
        reader.onload = function (e) {
            ...
            self.xhrUpload({
                ...
                errorfn: function (xhr) {
                    clearInput();
                    if (E.isOnWebsite) {
                        alert('JEditor官网暂时没有服务端，因此报错。实际项目中不会发生');
                    }
                    // 执行配置中的方法
                    var editor = this;
                ...
                timeoutfn: function (xhr) {
                    clearInput();
                    if (E.isOnWebsite) {
                        alert('JEditor官网暂时没有服务端，因此超时。实际项目中不会发生');
                    }
                    // 执行配置中的方法
                    var editor = this;
...
_e(function (E, $) {
    ...
    UploadFile.fn.render = function () {
        var self = this;
        var editor = self.editor;
        var uploadFileName = editor.config.uploadImgFileName || 'JEditorFormFile';
        if (self._hasRender) {
            // 不要重复渲染
            return;
        }
    ...
    UploadFile.fn.bindLoadEvent = function () {
        ...
        var onload = self.uploadFns.onload || editor.config.uploadImgFns.onload;
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var config = editor.config;
        var uploadImgUrl = config.uploadImgUrl;
        var uploadFileName = config.uploadImgFileName || 'JEditorPasteFile';
        var pasteEvent;
        var $imgsBeforePaste;
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var config = editor.config;
        var uploadImgUrl = config.uploadImgUrl;
        var uploadFileName = config.uploadImgFileName || 'JEditorDragFile';
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var $triangle = $('<div class="tip-triangle"></div>');
        var $delete = $('<a href="#"><i class="jeditor-icon-trash-o"></i></a>');
        var $zoomSmall = $('<a href="#"><i class="jeditor-icon-search-minus"></i></a>');
        var $zoomBig = $('<a href="#"><i class="jeditor-icon-search-plus"></i></a>');
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var $zoomSmall = $('<a href="#"><i class="jeditor-icon-search-minus"></i></a>');
        var $zoomBig = $('<a href="#"><i class="jeditor-icon-search-plus"></i></a>');
        // var $floatLeft = $('<a href="#"><i class="jeditor-icon-align-left"></i></a>');
        // var $noFloat = $('<a href="#"><i class="jeditor-icon-align-justify"></i></a>');
        // var $floatRight = $('<a href="#"><i class="jeditor-icon-align-right"></i></a>');
        var $alignLeft = $('<a href="#"><i class="jeditor-icon-align-left"></i></a>');
        var $alignCenter = $('<a href="#"><i class="jeditor-icon-align-center"></i></a>');
        var $alignRight = $('<a href="#"><i class="jeditor-icon-align-right"></i></a>');
        var $link = $('<a href="#"><i class="jeditor-icon-link"></i></a>');
        var $unLink = $('<a href="#"><i class="jeditor-icon-unlink"></i></a>');
        ...
        function bindToolbarEvent() {
            ...
            $alignLeft.click(function (e) {
                commandFn = function () {
                    ...
                    $currentImg.closest('p,div,section').css({
                        'text-align': 'left'
                    }).attr('align', 'left');
                };
                customCommand(e, function () {
...
_e(function (E, $) {
    E.plugin(function () {
        ...
        var $toolbar = $('<div class="txt-toolbar"></div>');
        var $triangle = $('<div class="tip-triangle"></div>');
        var $triggerLink = $('<a href="#" target="_blank"><i class="jeditor-icon-link"></i> ' + lang.openLink + '</a>');
        var isRendered;
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        var menu = new E.Menu({
            ...
            $domNormal: $('<a href="#" tabindex="-1"><i class="jeditor-icon-indent-left"></i></a>'),
            $domSelected: $('<a href="#" tabindex="-1" class="selected"><i class="jeditor-icon-indent-left"></i></a>')
        });
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        var menu = new E.Menu({
            ...
            // 正常状态和选中装下的dom对象，样式需要自定义
            $domNormal: $('<a href="#" tabindex="-1"><i class="jeditor-icon-arrows-v"></i></a>'),
            $domSelected: $('<a href="#" tabindex="-1" class="selected"><i class="jeditor-icon-arrows-v"></i></a>')
        });
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        // UI
        var $uploadIcon = $('<a href="#" class="upload-icon-container"><i class="jeditor-icon-upload"></i></a>');
        $uploadContent.append($uploadIcon);
...
// 版权提示
_e(function (E, $) {
    E.info('本页面富文本编辑器由 JEditor 提供 http:// ');
});
    
    // 最终返回JEditor构造函数
    return window.JEditor;
});
(function($){
